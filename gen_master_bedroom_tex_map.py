"""
gen_master_bedroom_tex_map.py — generate src/master_bedroom_tex_map.h from
"Master Bedroom.smx". Same name-based scheme as gen_conservatory_tex_map.py (the
Blender exporter renumbers the SMX's own texture list whenever the material
set changes, so raw indices are unstable).

NAME_TO_SLOT MUST match the slot order loaded in src/master_bedroom.c
(master_bedroom_load_assets). If you add/move a texture slot there, update it
here too.

Usage:
    python gen_master_bedroom_tex_map.py     # assets/Master Bedroom.smx -> src/master_bedroom_tex_map.h
    python gen_master_bedroom_tex_map.py <smx> <out>
"""
import xml.etree.ElementTree as ET
import sys

SMX = sys.argv[1] if len(sys.argv) > 1 else 'assets/Master Bedroom.smx'
OUT = sys.argv[2] if len(sys.argv) > 2 else 'src/master_bedroom_tex_map.h'

# Engine VRAM slot for each texture name (must match src/master_bedroom.c load order).
NAME_TO_SLOT = {
    'wd_flr':     0,
    'red_wlppr':  1,
    'din_cl':     2,
    ...
    if n is None:
        ...
        if name not in NAME_TO_SLOT:
            sys.exit(f"ERROR: SMX texture '{name}' has no engine slot in NAME_TO_SLOT "
                     f"(update both this script and src/master_bedroom.c).")
...
lines = [
    "/* Auto-generated by gen_master_bedroom_tex_map.py — do not edit.",
    "   Per-poly texture mapped by NAME to engine VRAM slots (see the script). */",
    "#pragma once",
    "#include <stdint.h>",
    f"#define MASTER_BEDROOM_PRIM_COUNT {len(entries)}",
    f"static const uint8_t master_bedroom_tex_map[{len(entries)}] = {{",
]
row = []
...
lines.append(f"static const uint8_t master_bedroom_nocull[{len(nocull)}] = {{")
row = []
