"""
gen_west_corridor_tex_map.py — generate src/west_corridor_tex_map.h from
"West Corridor.smx". Same name-based scheme as gen_master_bedroom_tex_map.py (the
Blender exporter renumbers the SMX's own texture list whenever the material
set changes, so raw indices are unstable).

NAME_TO_SLOT MUST match the slot order loaded in src/west_corridor.c
(west_corridor_load_assets). If you add/move a texture slot there, update it
here too.

Usage:
    python gen_west_corridor_tex_map.py     # assets/West Corridor.smx -> src/west_corridor_tex_map.h
    python gen_west_corridor_tex_map.py <smx> <out>
"""
import xml.etree.ElementTree as ET
import sys

SMX = sys.argv[1] if len(sys.argv) > 1 else 'assets/West Corridor.smx'
OUT = sys.argv[2] if len(sys.argv) > 2 else 'src/west_corridor_tex_map.h'

# Engine VRAM slot for each texture name (must match src/west_corridor.c load order).
NAME_TO_SLOT = {
    'red_wlppr':   0,
    'wd_flr':      1,
    'din_cl':      2,
    ...
    if n is None:
        ...
        if name not in NAME_TO_SLOT:
            sys.exit(f"ERROR: SMX texture '{name}' has no engine slot in NAME_TO_SLOT "
                     f"(update both this script and src/west_corridor.c).")
...
lines = [
    "/* Auto-generated by gen_west_corridor_tex_map.py — do not edit.",
    "   Per-poly texture mapped by NAME to engine VRAM slots (see the script). */",
    "#pragma once",
    "#include <stdint.h>",
    f"#define WEST_CORRIDOR_PRIM_COUNT {len(entries)}",
    f"static const uint8_t west_corridor_tex_map[{len(entries)}] = {{",
]
row = []
...
lines.append(f"static const uint8_t west_corridor_nocull[{len(nocull)}] = {{")
row = []
