"""
gen_library_tex_map.py - generate src/library_tex_map.h from
...
NAME_TO_SLOT MUST match the slot order loaded in src/library.c
(library_load_assets). If you add/move a texture slot there, update it
here too.

Usage:
    python gen_library_tex_map.py     # assets/Library.smx -> src/library_tex_map.h
    python gen_library_tex_map.py <smx> <out>
"""
import xml.etree.ElementTree as ET
import sys

SMX = sys.argv[1] if len(sys.argv) > 1 else 'assets/Library.smx'
OUT = sys.argv[2] if len(sys.argv) > 2 else 'src/library_tex_map.h'

# Engine VRAM slot for each texture name (must match src/library.c load order).
NAME_TO_SLOT = {
    'cncrte':     0,
    'prpl_wlppr': 1,
    'wd_flr':     2,
...
lines = [
    "/* Auto-generated by gen_library_tex_map.py - do not edit.",
    "   Per-poly texture mapped by NAME to engine VRAM slots (see the script). */",
    "#pragma once",
    "#include <stdint.h>",
    f"#define LIBRARY_PRIM_COUNT {len(entries)}",
    f"static const uint8_t library_tex_map[{len(entries)}] = {{",
]
row = []
...
lines.append(f"static const uint8_t library_nocull[{len(nocull)}] = {{")
row = []
