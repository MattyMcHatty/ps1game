"""
gen_garden_stairs_tex_map.py - generate src/garden_stairs_tex_map.h from
"Garden Stairs.smx". Same name-based scheme as gen_attic_stairwell_tex_map.py (the
Blender exporter renumbers the SMX's own texture list whenever the material
set changes, so raw indices are unstable).

NAME_TO_SLOT MUST match the slot order loaded in src/garden_stairs.c
(garden_stairs_load_assets). If you add/move a texture slot there, update it
here too.

Usage:
    python gen_garden_stairs_tex_map.py       # assets/Garden Stairs.smx ->
                                           # src/garden_stairs_tex_map.h
    python gen_garden_stairs_tex_map.py <smx> <out>
"""
import xml.etree.ElementTree as ET
import sys

SMX = sys.argv[1] if len(sys.argv) > 1 else 'assets/Garden Stairs.smx'
OUT = sys.argv[2] if len(sys.argv) > 2 else 'src/garden_stairs_tex_map.h'
    ...
    if n is None:
        ...
        if name not in NAME_TO_SLOT:
            sys.exit(f"ERROR: SMX texture '{name}' has no engine slot in NAME_TO_SLOT "
                     f"(update both this script and src/garden_stairs.c).")
...
lines = [
    "/* Auto-generated by gen_garden_stairs_tex_map.py - do not edit.",
    "   Per-poly texture mapped by NAME to engine VRAM slots (see the script). */",
    "#pragma once",
    "#include <stdint.h>",
    f"#define GARDEN_STAIRS_PRIM_COUNT {len(entries)}",
    f"static const uint8_t garden_stairs_tex_map[{len(entries)}] = {{",
]
row = []
...
lines.append(f"static const uint8_t garden_stairs_nocull[{len(nocull)}] = {{")
row = []
