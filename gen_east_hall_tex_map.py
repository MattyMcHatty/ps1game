"""
gen_east_hall_tex_map.py — generate src/east_hall_tex_map.h from
"East Hall.smx". Same name-based scheme as gen_master_bedroom_tex_map.py (the
Blender exporter renumbers the SMX's own texture list whenever the material
set changes, so raw indices are unstable).

NAME_TO_SLOT MUST match the slot order loaded in src/east_hall.c
(east_hall_load_assets). If you add/move a texture slot there, update it
here too.

Usage:
    python gen_east_hall_tex_map.py     # assets/East Hall.smx -> src/east_hall_tex_map.h
    python gen_east_hall_tex_map.py <smx> <out>
"""
import xml.etree.ElementTree as ET
import sys

SMX = sys.argv[1] if len(sys.argv) > 1 else 'assets/East Hall.smx'
OUT = sys.argv[2] if len(sys.argv) > 2 else 'src/east_hall_tex_map.h'

# Engine VRAM slot for each texture name (must match src/east_hall.c load order).
NAME_TO_SLOT = {
    'cncrte':     0,
    'wd_flr':     1,
    'inr_dbl_dr': 2,
    ...
    if n is None:
        ...
        if name not in NAME_TO_SLOT:
            sys.exit(f"ERROR: SMX texture '{name}' has no engine slot in NAME_TO_SLOT "
                     f"(update both this script and src/east_hall.c).")
...
lines = [
    "/* Auto-generated by gen_east_hall_tex_map.py — do not edit.",
    "   Per-poly texture mapped by NAME to engine VRAM slots (see the script). */",
    "#pragma once",
    "#include <stdint.h>",
    f"#define EAST_HALL_PRIM_COUNT {len(entries)}",
    f"static const uint8_t east_hall_tex_map[{len(entries)}] = {{",
]
row = []
...
lines.append(f"static const uint8_t east_hall_nocull[{len(nocull)}] = {{")
row = []
