...
/* =========================================================================
   ...
   =========================================================================
   THE ROOM IS BUILT. Mesh, textures, collision and the boss's own eight parts
   are all in; what is still missing is the FIGHT — no phases, no attacks, no
   camera, no sounds, no music. Every system the encounter needs was wired and
   budgeted before any of it existed, so what is left is a director module and
   ...
   from assets/garden/asag/Asag_Arena.smx: 674 TEXTURED primitives over
   x[-1500,1500] z[0,3698], floor y=0, perimeter walls to y=-1000 and detail up
   to y=-1624. It is read into the shared room arena on entry and drawn by this
   module's own loop, which is chain_room.c's.

   COLLISION FOLLOWS IT, and follows the BOSS too:
   src/asag_arena_mesh_collision.c is generated by
   tools/gen_asag_arena_collision.py out of THREE meshes — the room proxy, plus
   both tentacles' own geometry, because the arms lie along the side walls and
   the player must not walk through them. 48 walls against a cap of 128.

   THE ROOM IS A SEALED PIT WITH ONE RECESS. The proxy's only opening is the
   ALCOVE at x[-300,300] z[2613,3248], which is where Asag's head sits — it is
   the boss's throat, not a door.

   >>> SO THE EXIT HAS NO GEOMETRY, AND ITS POSITION IS A PLACEHOLDER. <<< The
   previous mesh had a door in the middle of the south wall; this one does not
   have a door anywhere. AA_EXIT_X/Z below therefore sit at the shaft mouth the
   player fell through, which at least reads as "climb back out", and main.c's
   destination for it is already flagged as a placeholder in the same words.
   Both move together when the way out of this room is decided.

   The player lands under the shaft mouth at the NORTH edge (low z) and faces
   +Z, straight down the arena at Asag.
   ...
   ---- WHAT IS STILL MISSING (all of it is the FIGHT) ------------------------
     - the encounter           no director module (src/asag_boss.c). The body
                               and its fourteen clips exist and NONE of them
                               play: see src/asag.h and asag_play().
     - the exit                where the room actually leads, and the geometry
                               of the way out. See THE GEOMETRY above.
...
/* ---- The streamed texture slots -------------------------------------------
   ...
   three names above the line are the ROOM's; the four below are ASAG'S SKINS,
   streamed here rather than registered by src/asag.c.

   >>> THE BOSS'S SKINS LIVE IN THE ROOM'S TABLE ON PURPOSE. <<< texmgr keeps
   the whole TIM in main RAM for the life of the run, so four registrations
   would be ~66 KB permanent for art that can only be seen in one room reached
   by a one-way drop. This room already makes a bracketed CD read on the only
   transition that gets here, and four more TIMs on it cost nothing at rest. */
#define ASAG_TEX_MUD    0    /* the arena floor and its mud banks   */
#define ASAG_TEX_WALL   1    /* "Boss Wall", the perimeter          */
#define ASAG_TEX_CHAIN  2    /* chain_128, 4bpp                     */
#define ASAG_TEX_SKIN   3    /* Asag's head                         */
#define ASAG_TEX_LEAF   4    /* the four leaves                     */
#define ASAG_TEX_TENT   5    /* both tentacles                      */
#define ASAG_TEX_BOIL   6    /* the six boils                       */

/* Read back what the entry-time stream captured from each TIM. Returns 0 for a
   slot that never loaded, which draws the part in whatever art sits at tpage 0
   rather than crashing — the same fallback a failed texmgr registration has. */
