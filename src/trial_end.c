...
/* ------------------------------------------------------------------- timing */
#define TE_T_FADE        240   /* 4.0 s, as briefed: the shaft -> purple       */
...
void trial_end_start(void) {
    ...
    /* THE MUSIC IS THE PIANO ROOM'S POST-ANZU TRACK, as briefed. It comes in on
       the first frame of the fade rather than when the text arrives, so that the
       cut from The Hatch's own track to the sign-off happens under the picture
       going out rather than in the silence after it. cdaudio_play switches
       tracks outright and re-asserts full volume, so nothing here has to stop
       what was playing first, nor undo the drive-mixer fades the opening
       sequence leaves. */
...
void trial_end_update(void) {
    ...
    /* Start is dead until the whole screen is up. There is no way back from
       here, so an accidental press — the player still had the pad in hand a
       moment ago — must not be able to throw the sign-off away. */
