...
/* Three the opening sequence itself never uses — a handle, a contraction and an
   exclamation — added for the sign-off screen (src/trial_end.c), which needs all
   three in "@ELECTRICRELOAD", "WHAT'S TO COME" and "THANK YOU!". Same
   convention: bit 0 is the left column. */
...
/* Digits, added for the sign-off's debug-mode hint, which has to name the
   shoulder buttons: "L1, L2, R1, R2". The whole set rather than the two that
   line happens to need, because a font carrying a 1 and a 2 and no other
   numeral is a trap for whoever writes the next line. Same 5x7 cell, same
   convention: bit 0 is the left column. */
...
/* One line, horizontally centred, at brightness `level` (0-255), into OT bucket
   ...
   PUBLIC because the sign-off screen (src/trial_end.c) fades text on in exactly
