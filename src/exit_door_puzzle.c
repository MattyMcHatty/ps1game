...
/* "Press O to remove the keystones" is 31 chars, which at 6*3 = 18 world units
   per char is 558 wide and would hang 54 units off each end of the 450-wide
   panel. At pixel 2 it is 372 and sits inside it, like every other sign here. */
#define XD_TEXT_PIXEL_SM          2
...
typedef enum {
    ...
    XD_QUAKE       /* the stones are out: hold, then shake. No board.      */
} XdState;
...
/* The stones have been pulled back out and the door is shut for good. Note this
   does NOT clear FLAG_EXIT_DOOR_UNLOCKED — that bit is still the record that the
   puzzle was solved, and leaving it set is what keeps the board from re-opening
   on the now-empty sockets. Every read below tests this one FIRST. */
static int stones_out(void) { return game_flag(FLAG_HADAD_TWO); }

/* The one window in which Circle pulls the stones: the door is open, Hadad has
   come off his plinth at least once, and they are still in the door. */
static int removable(void) {
    return solved() && game_flag(FLAG_HADAD_ONE) && !stones_out();
}
...
/* ---- Taking them back out ---------------------------------------------------
   Circle on the open door once Hadad has been off his plinth. All four stones
   come out — the three the player spent AND the magenta one that was part of the
   lock from the start — and the door shuts behind them, from both sides, for
   good (garden_stairs.c reads the same flag on its face of it).

   Runs from XD_IDLE, i.e. from free play with the player's own camera live, so
   there is nothing to save but that camera and nothing to glide: the quake
   shakes around the spot they are standing on and puts it back. */
static void remove_keystones(void) {
    player_items |= (1 << ITEM_GREEN_KEY_STONE)  |
                    (1 << ITEM_YELLOW_KEY_STONE) |
                    (1 << ITEM_BLUE_KEY_STONE)   |
    ...
    attic_exit_relock_door();          /* swap xt_dr_cmplt -> xt_dr_lckd */
...
void exit_door_puzzle_update(void) {
    ...
    if (state == XD_IDLE) {
        ...
        /* Four outcomes off the one press, most-final first:
             stones out  — dead. The sign reads "Locked" and this is a wall now.
             removable   — pull the four stones; the quake follows.
             solved      — the way OUT, not back into a finished board.
             otherwise   — open the board. */
        if      (stones_out()) return;
        else if (removable())  remove_keystones();
        else if (solved())     exit_latch = 1;
        else                   start_puzzle();
        return;
    }
...
void exit_door_puzzle_draw(RenderContext *ctx) {
    /* XD_QUAKE joins the two states with nothing on screen: it owns the camera
       but draws no board at all — the room shaking IS the whole of it. */
    if (state == XD_IDLE || state == XD_INTRO || state == XD_QUAKE) return;
...
void exit_door_prompt_draw(RenderContext *ctx) {
    ...
    /* Same four cases as the Circle dispatch in the update, in the same order —
       and they must stay in step, or the sign will offer something the press
       does not do. Red and no button glyph for the dead one, matching
       Reception's "Locked from the other side" (reception.c). */
    if (stones_out()) {
        door_draw_string_3d(ctx, "Locked",
                            XD_DOOR_X - 200, XD_TEXT_Y, XD_DOOR_Z - 11,
                            255, 50, 50, fade, 0, TEXT_PLANE_XY, XD_TEXT_PIXEL);
        return;
    }
    if (removable()) {
        door_draw_string_3d(ctx, "Press " BTN_CIRCLE " to remove the keystones",
                            XD_DOOR_X - 200, XD_TEXT_Y, XD_DOOR_Z - 11,
                            50, 255, 50, fade, 0, TEXT_PLANE_XY, XD_TEXT_PIXEL_SM);
        return;
    }
