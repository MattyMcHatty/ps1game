"""
gen_east_stairwell_tex_map.py - generate src/east_stairwell_tex_map.h from
"East Stairwell.smx". Same name-based scheme as gen_library_tex_map.py (the
Blender exporter renumbers the SMX's own texture list whenever the material
set changes, so raw indices are unstable).

NAME_TO_SLOT MUST match the slot order loaded in src/east_stairwell.c
(east_stairwell_load_assets). If you add/move a texture slot there, update it
here too.

Usage:
    python gen_east_stairwell_tex_map.py   # assets/East Stairwell.smx ->
                                           # src/east_stairwell_tex_map.h
    python gen_east_stairwell_tex_map.py <smx> <out>
"""
import xml.etree.ElementTree as ET
import sys

SMX = sys.argv[1] if len(sys.argv) > 1 else 'assets/East Stairwell.smx'
OUT = sys.argv[2] if len(sys.argv) > 2 else 'src/east_stairwell_tex_map.h'

# Engine VRAM slot for each texture name (must match src/east_stairwell.c load order).
NAME_TO_SLOT = {
    'cncrte':   0,
    'wd_flr':   1,
    'wd_dr':    2,
    ...
    if n is None:
        ...
        if name not in NAME_TO_SLOT:
            sys.exit(f"ERROR: SMX texture '{name}' has no engine slot in NAME_TO_SLOT "
                     f"(update both this script and src/east_stairwell.c).")
...
lines = [
    "/* Auto-generated by gen_east_stairwell_tex_map.py - do not edit.",
    "   Per-poly texture mapped by NAME to engine VRAM slots (see the script). */",
    "#pragma once",
    "#include <stdint.h>",
    f"#define EAST_STAIRWELL_PRIM_COUNT {len(entries)}",
    f"static const uint8_t east_stairwell_tex_map[{len(entries)}] = {{",
]
row = []
...
lines.append(f"static const uint8_t east_stairwell_nocull[{len(nocull)}] = {{")
row = []
